...
static inline size_t shdr_get_size(const struct shdr* shdr)
{
    ...
    if (ADD_OVERFLOW(s, shdr->hash_size, &s) || ADD_OVERFLOW(s, shdr->sig_size, &s))
        return 0;
...
static inline size_t shdr_enc_get_size(const struct shdr_encrypted_ta* ehdr)
{
    ...
    if (ADD_OVERFLOW(s, ehdr->iv_size, &s) || ADD_OVERFLOW(s, ehdr->tag_size, &s))
        return 0;
...
#define SHDR_ENC_GET_IV(x) ((uint8_t*)(((struct shdr_encrypted_ta*)(x)) + 1))
