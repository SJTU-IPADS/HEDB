...
public:
  unsigned set_total_test(unsigned int a ){
    if(a == 0)
      return m_numTotalTests;
    m_numTotalTests = a;
    return a;
  }
...
int main(int argc, char **argv, char **envp) {
  ...
  if (true) {
    ...
    for (std::vector<std::string>::iterator
         ...
         it != ie; ++it) { // insert all seeds in seed-dir
      std::vector<std::string> kTestFiles;
      KleeHandler::getKTestFilesInDir(*it, kTestFiles);
      std::vector<int> nums;
      std::transform(kTestFiles.begin(), kTestFiles.end(), std::back_inserter(nums), [](const std::string &str){
          int index = 0;
          while(str[index] < '0' || str[index] > '9'){
            index ++;
          }
          std::stringstream ss(str.substr(index));
          int n;
          ss >> n;
          return n;
      });
      klee_message("totoal size of files %d\n", nums.size());
      ...
      for(int i = 0; i < kTestFiles.size(); i ++){     
        KTest *out = kTest_fromFile(kTestFiles[indices[i]].c_str());
        if (!out) {
          klee_error("unable to open: %s\n", kTestFiles[indices[i]].c_str());
        }
        seeds.push_back(out);
      }
      if (kTestFiles.empty()) {
    ...
    //   std::vector<KTest *> tmp_seed;
    //   tmp_seed.push_back(seed);

    //     klee_message("KLEE: %d run, using %lu seeds\n",cnt ++, tmp_seed.size());
    //   if (!tmp_seed.empty()) {
    //       interpreter->useSeeds(&tmp_seed);
    ...
    {
      int index = 0;
      while(tmp[index] < '0' || tmp[index] > '9'){
        index ++;
      }
      std::stringstream ss(tmp.substr(index));
    ...
    int pgrp_magic = getpid();
    for(int i = start_point; i < seeds.size() && i < start_point + 10000; i ++){
        std::vector<KTest *> tmp_seed;
        tmp_seed.push_back(seeds[i]); 
        current_parallel ++;

        while(current_parallel > max_parallel){
          waitpid(-pgrp_magic, 0, 0); // at least one
          current_parallel--;
          while(waitpid(-pgrp_magic, 0, WNOHANG) > 0) // 
            current_parallel--;
        }
        pid_t pid = fork();
        if(pid < 0){
          printf("fork failed here!!!!, exiting %d, pid %d\n",i, pid);
          //
          exit(0);
        }
        if(pid == 0){
          setpgid(0, pgrp_magic);
          handler->set_total_test(i);
          interpreter->useSeeds(&tmp_seed);
          interpreter->runFunctionAsMain(mainFn, pArgc, pArgv, pEnvp);
          exit(0);
        }
    ...
    // interpreter->runFunctionAsMain(mainFn, pArgc, pArgv, pEnvp);
    while(current_parallel > 0){
      waitpid(-pgrp_magic, 0, 0);
      current_parallel--;
    }
    return 0;
    /* the main function is runned with seeds. */
    while (!seeds.empty()) { // avoid memleak, just free seeds here
      kTest_free(seeds.back());
      seeds.pop_back();
    }
  }

  return 0; // exit means free ~
