...
void Executor::branch(ExecutionState &state,
  ...
  /* check if currently allow to branch 
    if not allowed, only check first branch.
  */
  ...
  if (it != seedMap.end()) {
    ...
    for (std::vector<SeedInfo>::iterator siit = seeds.begin(), 
      ...
      if (i==N){
        assert(0); // have to match a condition
        i = theRNG.getInt32() % N;
      }
...
Executor::StatePair Executor::fork(ExecutionState &current, ref<Expr> condition,
  ...
  if (isSeeding && 
    ...
    /* if only one branch have seed in ors mode,
        addConstraint to state without creating new.
     */
    if (!(trueSeed && falseSeed)) {
      assert(trueSeed || falseSeed);
      res = trueSeed ? Solver::True : Solver::False;
      addConstraint(current, trueSeed ? condition : Expr::createIsZero(condition));
      // std::cout << trueSeed << std::endl;
    }
  }
...
ref<Expr> Executor::toUnique(const ExecutionState &state, 
  ...
  if (!isa<ConstantExpr>(e)) {
    ...
    ExecutionState* pstate = const_cast<klee::ExecutionState*>(&state);
    auto *seed = &seedMap[pstate][0];
    if (solver->getValue(state.constraints, seed->assignment.evaluate(e), value, state.queryMetaData)) {
      ref<Expr> cond = EqExpr::create(e, value);
      cond = optimizer.optimizeExpr(cond, false);
      if (solver->mustBeTrue(state.constraints, seed->assignment.evaluate(cond), isTrue,
                             state.queryMetaData) &&
          isTrue)
        result = value;
    }
...
Function* Executor::getTargetFunction(Value *calledVal, ExecutionState &state) {
  ...
  if (!c){
    // std::cout << "non constant called value %p: " << calledVal <<std::endl;
    return 0;
  }
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::Call: {
    ...
    if (f) {
      ...
      // assert(0); // is this really happending ? 
      /* we can evaluate with the seed and take the branch  */
      ref<Expr> v = eval(ki, 0, state).value;
      // ref<ConstantExpr> value;
      // auto* pstate = &state;
      // auto* seed = &seedMap[pstate][0];
      // bool success =
      //       solver->getValue(pstate->constraints, seed->assignment.evaluate(v), value, pstate->queryMetaData);
      
      // addConstraint(state, EqExpr::create(v, value));

      // uint64_t addr = value->getZExtValue();
      // auto it = legalFunctions.find(addr);
      // assert(it != legalFunctions.end());
      // f = it->second;
  ...
  for (std::vector<ref<Expr> >::iterator ai = arguments.begin(), 
    ...
    if (ExternalCalls == ExternalCallPolicy::All) { // don't bother checking uniqueness
      ...
      *ai = optimizer.optimizeExpr(*ai, true);
      ...
      ref<Expr> arg = toUnique(state, *ai); // should be constant
...
void Executor::executeMemoryOperation(ExecutionState &state,
  ...
  if (success) {
    ...
    auto* seed = &seedMap[&state][0];
    solver->setTimeout(coreSolverTimeout);
    bool success = solver->mustBeTrue(state.constraints, seed->assignment.evaluate(check), inBounds,
                                      state.queryMetaData);
    solver->setTimeout(time::Span());
    if (!success) {
      state.pc = state.prevPC;
...
bool Executor::getSymbolicSolution(const ExecutionState &state,
                                   ...
                                   &res) {
  // std::cout << "getting result symbolic solution " << std::endl;
  solver->setTimeout(coreSolverTimeout);
